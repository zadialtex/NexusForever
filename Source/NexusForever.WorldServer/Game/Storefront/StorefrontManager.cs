    ...
    public static class StorefrontManager
    {
        ...
        private static void InitialiseStoreOfferGroups()
        {
            ...
            StoreOfferGroupList = StoreOfferGroups
                ...
                {
                    Id = x.Id,
                    DisplayFlags = (DisplayFlag)x.Field1,
                    OfferGroupName = x.Name,
                    OfferGroupDescription = x.Description,
                    Unknown2 = x.Field2,
                    Offers = GetOffersForGroup(x.Id),
                    ArraySize = (uint)(CalculateCategoryArray(x.CategoryId1, x.CategoryId2, x.CategoryId3).Length / 4),
                    CategoryArray = CalculateCategoryArray(x.CategoryId1, x.CategoryId2, x.CategoryId3),
                    CategoryIndexArray = CalculateCategoryArray(x.Category1Index, x.Category2Index, x.Category3Index, true)
                })
                .ToImmutableList();            
        }

        private static byte[] CalculateCategoryArray(uint cat1, uint cat2, uint cat3, bool indexCheck = false)
        {
            List<byte> CategoryArray = new List<byte>();
            uint[] CategoriesToCheck = new uint[] { cat3, cat2, cat1 };

            foreach(uint category in CategoriesToCheck)
            {
                if (category > 0)
                {
                    if (category == 26 && !indexCheck)
                        continue;

                    CategoryArray.Add((byte)category);
                    CategoryArray.Add(0);
                    CategoryArray.Add(0);
                    CategoryArray.Add(0);
                }
